...
- 資料庫連線池：`internal/db` 使用 pgx 預設的 server-side prepared-statement cache。若在 API 與 PostgreSQL 之間放 **PgBouncer**，必須用 **session pooling mode**；transaction/statement pooling 會破壞 prepared statement。要用 transaction pooling 時，改在 `internal/db.OpenPostgres` 設 `poolConfig.ConnConfig.DefaultQueryExecMode = pgx.QueryExecModeDescribeExec` 並完整跑一次 `-tags integration` 測試。
- 監控告警：`sta_http_*` 指標之外，至少對 outbox 轉入 `abandoned` 終態、login 失敗率、rate-limiter 失效、ClamAV 掃描失敗設告警（見 [security.md](security.md)）。
