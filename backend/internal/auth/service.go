...
func (s *Service) Authenticate(ctx context.Context, request *http.Request) (RequestSession, error) {
	...
	// Admin MFA is enforced by URL prefix: every admin-privileged endpoint MUST
	// be mounted under /api/v1/admin/ so this check fires. Handlers additionally
	// run repository.IsAdmin, but MFA enforcement lives only here — a privileged
	// route on any other prefix would silently skip the second factor.
